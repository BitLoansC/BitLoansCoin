...
#include "ui_zbitloanscoincontroldialog.h"
...
std::list<std::string> ZBitLoansControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitLoansControlDialog::listMints;

ZBitLoansControlDialog::ZBitLoansControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitLoansControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitLoansControlDialog::~ZBitLoansControlDialog()
{
    delete ui;
}

void ZBitLoansControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitLoansControlDialog::updateList()
{
...
void ZBitLoansControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitLoansControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitLoans_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitLoansControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitLoansControlDialog::GetSelectedMints()
{
...
void ZBitLoansControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
