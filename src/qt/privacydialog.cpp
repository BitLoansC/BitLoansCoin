...
#include "zbitloanscoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBLON ought to be enough for anybody." - Bill Gates, 2017
    ui->zBLONpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBLONSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBLON->setEnabled(false);
        ui->pushButtonMintzBLON->setToolTip(tr("zBLON is currently disabled due to maintenance."));

        ui->pushButtonSpendzBLON->setEnabled(false);
        ui->pushButtonSpendzBLON->setToolTip(tr("zBLON is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBLONpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBLON_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLON is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBLON...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBLON in ") + 
...
void PrivacyDialog::on_pushButtonSpendzBLON_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLON is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBLON
        sendzBLON();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBLON
    sendzBLON();
}

void PrivacyDialog::on_pushButtonZBitLoansControl_clicked()
{
    ZBitLoansControlDialog* zBitLoansControl = new ZBitLoansControlDialog(this);
    zBitLoansControl->setModel(walletModel);
    zBitLoansControl->exec();
}

void PrivacyDialog::setZBitLoansControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBitLoansSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBLON()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BitLoans Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBLONpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBLON is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BLON </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBLON</b>";
    ...
    // use mints from zBitLoans selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBitLoansControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitLoansControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbitloanscoin selector in case it was used
    ZBitLoansControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBitLoans Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BitLoans, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBitLoans Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBLON </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBLON "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBLON "));
    ui->labelzBLONAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBLONSyncStatus->setVisible(fShow);
}
