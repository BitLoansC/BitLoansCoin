...
*Setup instructions for a gitian build of BLON using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BLON
...
- [Building BLON](#building-bitloanscoin)
...
Clone the git repositories for bitloanscoin and gitian and then checkout the bitloanscoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitloanscoin-crypto/bitloanscoin.git
cd bitloanscoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitloanscoin repository
...
To build BLON (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitloanscoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitloanscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitloanscoin-crypto/bitloanscoin
...
For example:
```bash
URL=https://github.com/crowning-/bitloanscoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitloanscoin=${COMMIT} --url bitloanscoin=${URL} ../bitloanscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitloanscoin=${COMMIT} --url bitloanscoin=${URL} ../bitloanscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitloanscoin=${COMMIT} --url bitloanscoin=${URL} ../bitloanscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitloanscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitloanscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitloanscoin-build.assert
```
...
[bitloanscoin/gitian.sigs](https://github.com/bitloanscoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
